...
from ecdsa.util import sigdecode_string, sigencode_string  # type: ignore
...
def lnencode(addr, privkey):
    if addr.amount:
        ...
        amount = addr.currency + _shorten_amount(amount)
    else:
    ...
    payload = bytearray([ord(c) for c in hrp]) + data.tobytes()
    sig = privkey.sign_recoverable(payload, sigencode=sigencode_string)
    ...
    print("SIG", sig)
    print("DATA", data)
    #rusty -> privkey = secp256k1.PrivateKey(bytes(unhexlify(privkey)))
    #sig = privkey.sign_digest_deterministic(bytearray([ord(c) for c in hrp]) + data.tobytes())
    #rusty -> sig = privkey.ecdsa_sign_recoverable(bytearray([ord(c) for c in hrp]) + data.tobytes())
    # This doesn't actually serialize, but returns a pair of values :(
    # sig, recid = sig[0], sig[1]
    # data += bytes(sig) + bytes([recid])

    return bech32_encode(hrp, _bitarray_to_u5(data))
